	        	...
	        	cv.readImage(src1,function(err1,im1){
	        		...
	        		for(var i=0; i<arr0.length;i=i+1){
	        			for(var j=0; j<arr0[i].length;j=j+1){
	        				...
	        				//console.log('Depois');
	        				//console.log(MSBytes);
	        				LSBytes=valor2&0xF0; //selecionar os 3 MS bits e zerar os 5 LS bit
	        				//console.log('Antes')
	        				//console.log(valor1);
	        				//console.log('Depois')
	        				//console.log(MSBytes);
	        				arrFinal[i][j][k]=(MSBytes|(LSBytes>>4)); //criando três canais devido a chamada da função
	        		 ...
	        		 for (var i=0; i<arrIn.length;i++){
	        		 	for(var j=0;j<arrIn[i].length;j++){
	        		 		for(var k=0;k<arrIn[i][j].length;k=k+1){
	        		 			arrMain[i][j][k] = arrIn[i][j][k]&0xF0;
	        		 			arrHide[i][j][k]=(arrIn[i][j][k]&0x0F)<<4;
